...
def create_database():
    ...
    for song_id, filename in zip(random_ids, os.listdir('Songs')):
        ...
        filtered_bins = song_recipe(os.path.join("Songs", filename))
...
def find_song(hash_dictionary, sample_dictionary, id_to_song):
        ...
        for sample_offset in sample_offsets:
            try:
                for song_id, offset in hash_dictionary[sample_hash_value]:
                    try:
                        offset_dictionary[song_id][(offset - sample_offset) // 1] += 1
                    except KeyError:
                        offset_dictionary[song_id][(offset - sample_offset) // 1] = 1
            except KeyError:
                pass
    ...
    for song_id, offset_dict in offset_dictionary.items():
        for relative_set, frequency in offset_dict.items():
            try:
                max_frequencies[song_id] = max(max_frequencies[song_id], frequency)
            except KeyError:
                max_frequencies[song_id] = frequency
    max_frequencies_keys = sorted(max_frequencies, key=max_frequencies.get, reverse=True)
    return max_frequencies, max_frequencies_keys
