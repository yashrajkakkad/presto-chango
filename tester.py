...
from database import hash_sample, create_database, load_database, find_song
...
def download_songs(playlist_url):
    command_string = 'youtube-dl -x --audio-format wav --postprocessor-args "-ar 44100 -ac 1" --output "Songs/%(title)s_%(id)s.%(ext)s" ' + \
        playlist_url
...
def gen_random_samples():
    for filename in os.listdir("Songs"):
        ...
        subprocess.call(['ffmpeg', '-i', os.path.join("Songs", filename),
                         '-ss', str(datetime.timedelta(seconds=start_point)), '-to', str(datetime.timedelta(seconds=end_point)), '-y', os.path.join("Song_Samples", filename)])
