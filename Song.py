...
def apply_window_function(data, window_size, window_function):
    windows = util.view_as_windows(data, window_shape=(window_size,), step=100) # (window_size,) ==> dimensions are window_size x 1
...
def filter_spectrogram(windows, window_size):
    ...
    # Init 2D list
    filtered_bins = [[0 for i in range(len(RANGES))] for j in range(len(windows))] # rows = no. of windows, cols = no. of bands
